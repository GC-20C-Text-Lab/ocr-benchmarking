...
class Entry(BaseModel):
    ...
    firstname: str = Field(
        default="",
        description="The author's first name(s). In the orginal entries, names are written in Last Name, First Name format. If this information is not found, assign the empty string to this field.",
    )
    ...
    publishyear: int = Field(
        default=0,
        description='The year the book was published. Usually seperated from the name of the publisher by a comma, although occasionally a period was used instead. In entries that include the words "No imprint", this information is often unavailable. If this information is not found, assign 0 to this field.',
    )
    pagecount: int = Field(
        default=0,
...
# async def process_img2json_async(input_imgs, output_dir, processor, doc_format, model):
